...
with torch.no_grad():
    ...
    emo_model.load_state_dict(torch.load('T5_emotion.pt', map_location=torch.device('cpu')))
...
def get_distance(s1, s2):
  '''
  Computes a distance score between utterances
  '''
...
def get_sentence_score(sentence, dataframe):
  '''
  Calculates how fit a sentence is based on its weighted fluency
  and novelty values
  '''
  fluency = fluency_score(sentence)
  novelty = novelty_score(sentence, dataframe)
  score = fluency + novelty
  return score
